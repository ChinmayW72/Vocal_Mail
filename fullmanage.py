...
@app.route('/')
def dashboard():
    speak("Welcome! Would you like to check your emails or compose a new mail?")
    command = get_audio()

    if 'check' in command and 'mails' in command:
        return redirect(url_for('check_mails'))
    elif 'compose' in command:
        return redirect(url_for('compose'))
    else:
        speak("I didn't understand that. Please say check mails or compose mail.")
...
def check_mails():
    ...
    email_snippets = []
    if not messages:
        speak("No messages found in your inbox.")
    ...
    # After fetching emails, pass them to the template
